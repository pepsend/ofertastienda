// build.js - v36 con Imágenes de Alta Calidad
import fs from 'fs';
import Papa from 'papaparse';
import fetch from 'node-fetch';

console.log("Iniciando proceso de construcción (v36 - con Imágenes HQ)...");
...
// ============================================================================
async function build() { /* ...código sin cambios... */
    let dataCache = {};
    ...
    for (let i = 0; i < csvData.length; i++) {
        ...
        if (!dataCache[game.Nombre.toLowerCase()]) { newCacheEntries++; dataCache[game.Nombre.toLowerCase()] = details; }
        finalGameData.push({ nombre: game.Nombre, precio: game.Precio, edicion: game.Edición, plataforma: game.Plataforma, oferta: game.Oferta, imageUrl: details.imageUrl, genres: details.genres });
...
// ============================================================================
//  FUNCIONES AUXILIARES (Con cambios en fetchFromIgdbApi)
// ============================================================================

async function getGameDetails(game, dataCache) { /* ...código sin cambios... */ 
    ...
    return finalData; // El caché se guarda en la función build ahora
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (data && data.length > 0) {
                const game = data[0];
                let imageUrl = null;
                // PRIORIDAD 1: Usar la portada grande (cover) si existe
                if (game.cover && game.cover.url) {
                    // Reemplazamos el tamaño por uno grande y de alta calidad para portadas
                    imageUrl = game.cover.url.replace('t_thumb', 't_cover_big');
                } 
                // PRIORIDAD 2: Si no hay portada, usar la primera screenshot en alta calidad
                else if (game.screenshots && game.screenshots.length > 0) {
                    // Reemplazamos el tamaño por uno grande y panorámico
...
// ... (El resto de funciones auxiliares como fetchFromRawgApi, getSimilarity, normalizeName son idénticas al v35)
